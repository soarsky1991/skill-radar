...
AI agent skills、MCP servers、prompts、context tools、agent-native CLIs 越来越多，但真正难的是判断哪些需求值得做、哪些只是热闹。

所以我做了 **Agent Skill Radar**。

它用公开 GitHub 信号生成每日 build brief：

1. 扫描 agent-skill、MCP、prompt、context-engineering、agent CLI 相关仓库；
2. 按热度、活跃度、issue 需求、扩展性、内容传播价值、新颖度和饱和度评分；
3. 输出 Markdown 报告，把“下一步可以做什么工具”讲清楚。

第一版故意很小：GitHub API、透明评分、每日 Markdown 报告。

我主要会围绕中文开发者、技术创作者和小团队继续运营它：用它做开源选题、内容选题、MCP/Agent 工具筛选和产品化验证。
...
Chinese-speaking builders are the first audience for community growth and monetization, but the project remains open and bilingual so it can connect with the global AI agent ecosystem.
